...
// Functional component to display doctors in a table
export function UDoctorsTable() {
  const { data } = useDoctors(); // Fetch doctors data using a custom hook
    ...
    <div>
      {data && (
        <table className="w-full table-auto">
          ...
          <tbody>
            {/* Map through the doctors data and create a row for each doctor */}
            {data.map((item, index) => (
              <tr
                key={item._id}
