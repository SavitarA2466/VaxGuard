...
// Importing necessary components, icons, hooks, and libraries
...
// Functional component AAddAppointmentModal
function AAddAppointmentModal({ closeModal, isOpen, datas }) {
  ...
  // Handler for changing share options
  const onChangeShare = (e) => {
    setShares({ ...shares, [e.target.name]: e.target.checked });
  };

  // Custom hooks to fetch children, services, and doctors data
  ...
  // Effect to populate form fields if editing an existing appointment
  useEffect(() => {
    if (datas?.title) {
...
// Exporting AAddAppointmentModal as default export
